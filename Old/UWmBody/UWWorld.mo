...
model UWWorld "World coordinate system + gravity field + underwater field + default animation definition"
  import UWmBody.UWTypes.GravityTypes;
  import Types = UWmBody.UWTypes;
  UWInterfaces.Frame_b frame_b "Coordinate system fixed in the origin of the world frame" annotation(Placement(transformation(extent = {{84, -16}, {116, 16}})));
  ...
  parameter UWTypes.AxisLabel label1 = "x" "Label of horizontal axis in icon";
  parameter UWTypes.AxisLabel label2 = "y" "Label of vertical axis in icon";
  parameter UWTypes.GravityTypes gravityType = GravityTypes.UniformGravity "Type of gravity field" annotation(Evaluate = true);
  ...
  parameter UWTypes.Axis n = {0, -1, 0} "Direction of gravity resolved in world frame (gravity = g*n/length(n))" annotation(Evaluate = true, Dialog(enable = gravityType == Modelica.Mechanics.MultiBody.Types.GravityTypes.UniformGravity));
  // parameter UWTypes.Axis n_b = -n "Direction of buoyant force, resolved in world frame (buoyant_f = b_f*n_b/length(n_b))" annotation(Evaluate = true, Dialog(enable = gravityType == Modelica.Mechanics.MultiBody.Types.GravityTypes.UniformGravity));
  ...
  // parameter SI.Density pho_uw(min = 0, start = 1) = 1000 "Density of surrounding fluid in underwater world" annotation(Evaluate = true);
  ...
  input UWTypes.Color axisColor_x = UWmBody.UWTypes.Defaults.FrameColor "Color of x-arrow" annotation(Dialog(colorSelector = true, tab = "Animation", group = "if animateWorld = true", enable = enableAnimation and animateWorld));
  input UWTypes.Color axisColor_y = axisColor_x annotation(Dialog(colorSelector = true, tab = "Animation", group = "if animateWorld = true", enable = enableAnimation and animateWorld));
  input UWTypes.Color axisColor_z = axisColor_x "Color of z-arrow" annotation(Dialog(colorSelector = true, tab = "Animation", group = "if animateWorld = true", enable = enableAnimation and animateWorld));
  ...
  input UWTypes.Color gravityArrowColor = {0, 230, 0} "Color of gravity arrow" annotation(Dialog(colorSelector = true, tab = "Animation", group = "if animateGravity = true and gravityType = UniformGravity", enable = enableAnimation and animateGravity and gravityType == GravityTypes.UniformGravity));
  ...
  input UWTypes.Color gravitySphereColor = {0, 230, 0} "Color of gravity sphere" annotation(Dialog(colorSelector = true, tab = "Animation", group = "if animateGravity = true and gravityType = PointGravity", enable = enableAnimation and animateGravity and gravityType == GravityTypes.PointGravity));
  ...
  replaceable function gravityAcceleration = UWmBody.UWForces.Internal.standardGravityAcceleration(gravityType = gravityType, g = g * Modelica.Math.Vectors.normalizeWithAssert(n), mue = mue) constrainedby UWmBody.UWInterfaces.partialGravityAcceleration "Function to compute the gravity acceleration, resolved in world frame" annotation(choicesAllMatching = true, Dialog(enable = gravityType == Modelica.Mechanics.MultiBody.Types.GravityTypes.NoGravity), Documentation(info = "<html>
  ...
  /* replaceable function buoyantAcceleration = UWmBody.UWForces.Internal.standardBuoyantAcceleration(g_a = g, dir = Modelica.Math.Vectors.normalizeWithAssert(n), pho_fluid = pho_uw) constrainedby UWmBody.UWInterfaces.partialBuoyantAcceleration "Function to compute the buoyant acceleration, resolved in world frame" annotation(choicesAllMatching = true, Dialog(enable = gravityType == Modelica.Mechanics.MultiBody.Types.GravityTypes.NoGravity), Documentation(info = "<html>
      <p>Replaceable function to define the gravity field.
         Default is function
         <a href=\"modelica://Modelica.Mechanics.MultiBody.Forces.Internal.standardGravityAcceleration\">standardGravityAcceleration</a>
         that provides a simple buoyant fields (parallel to the gravity field).
         By redeclaring this function, any type of buoyant field can be defined.
      </p>
      </html>")); */
  // replaceable function viscousDragForce = UWmBody.UWForces.Internal.standardViscousDrag constrainedby UWmBody.UWInterfaces.partialDragForce annotation(choicesAllMatching = true);
  // replaceable function hydroDynDragForce = UWmBody.UWForces.Internal.standardHydroDynamicDrag(pho_fluid = pho_uw) constrainedby UWmBody.UWInterfaces.partialDragForce annotation(choicesAllMatching = true);
  /* The World object can only use the UWmBody.UWVisualizers.Advanced.Shape model, but no
         other models in package UWmBody.UWVisualizers, since the other models access
         data of the "outer UWmBody.UWWorld world" object, i.e., there are
...
protected
  ...
  parameter SI.Length scaledLabel = UWmBody.UWTypes.Defaults.FrameLabelHeightFraction * axisDiameter;
  parameter SI.Length labelStart = 1.05 * axisLength;
  // x-axis
  UWmBody.UWVisualizers.Advanced.Shape x_arrowLine(shapeType = "cylinder", length = lineLength, width = lineWidth, height = lineWidth, lengthDirection = {1, 0, 0}, widthDirection = {0, 1, 0}, color = axisColor_x, specularCoefficient = 0) if enableAnimation and animateWorld;
  UWmBody.UWVisualizers.Advanced.Shape x_arrowHead(shapeType = "cone", length = headLength, width = headWidth, height = headWidth, lengthDirection = {1, 0, 0}, widthDirection = {0, 1, 0}, color = axisColor_x, r = {lineLength, 0, 0}, specularCoefficient = 0) if enableAnimation and animateWorld;
  UWmBody.UWVisualizers.Internal.Lines x_label(lines = scaledLabel * {[0, 0; 1, 1], [0, 1; 1, 0]}, diameter = axisDiameter, color = axisColor_x, r_lines = {labelStart, 0, 0}, n_x = {1, 0, 0}, n_y = {0, 1, 0}, specularCoefficient = 0) if enableAnimation and animateWorld and axisShowLabels;
  // y-axis
  UWmBody.UWVisualizers.Advanced.Shape y_arrowLine(shapeType = "cylinder", length = lineLength, width = lineWidth, height = lineWidth, lengthDirection = {0, 1, 0}, widthDirection = {1, 0, 0}, color = axisColor_y, specularCoefficient = 0) if enableAnimation and animateWorld;
  UWmBody.UWVisualizers.Advanced.Shape y_arrowHead(shapeType = "cone", length = headLength, width = headWidth, height = headWidth, lengthDirection = {0, 1, 0}, widthDirection = {1, 0, 0}, color = axisColor_y, r = {0, lineLength, 0}, specularCoefficient = 0) if enableAnimation and animateWorld;
  UWmBody.UWVisualizers.Internal.Lines y_label(lines = scaledLabel * {[0, 0; 1, 1.5], [0, 1.5; 0.5, 0.75]}, diameter = axisDiameter, color = axisColor_y, r_lines = {0, labelStart, 0}, n_x = {0, 1, 0}, n_y = {-1, 0, 0}, specularCoefficient = 0) if enableAnimation and animateWorld and axisShowLabels;
  // z-axis
  UWmBody.UWVisualizers.Advanced.Shape z_arrowLine(shapeType = "cylinder", length = lineLength, width = lineWidth, height = lineWidth, lengthDirection = {0, 0, 1}, widthDirection = {0, 1, 0}, color = axisColor_z, specularCoefficient = 0) if enableAnimation and animateWorld;
  UWmBody.UWVisualizers.Advanced.Shape z_arrowHead(shapeType = "cone", length = headLength, width = headWidth, height = headWidth, lengthDirection = {0, 0, 1}, widthDirection = {0, 1, 0}, color = axisColor_z, r = {0, 0, lineLength}, specularCoefficient = 0) if enableAnimation and animateWorld;
  UWmBody.UWVisualizers.Internal.Lines z_label(lines = scaledLabel * {[0, 0; 1, 0], [0, 1; 1, 1], [0, 1; 1, 0]}, diameter = axisDiameter, color = axisColor_z, r_lines = {0, 0, labelStart}, n_x = {0, 0, 1}, n_y = {0, 1, 0}, specularCoefficient = 0) if enableAnimation and animateWorld and axisShowLabels;
...
equation
  ...
  frame_b.r_0 = zeros(3);
  frame_b.R = UWFrames.nullRotation();

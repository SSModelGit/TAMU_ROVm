...
model BodyCylinder "Rigid body with cylinder shape. Mass and animation properties are computed from cylinder data and density (12 potential states)"
  ...
  parameter UWmBody.UWTypes.Axis lengthDirection = to_unit1(r - r_shape) "Vector in length direction of cylinder, resolved in frame_a" annotation(Evaluate = true);
  ...
  input UWmBody.UWTypes.Color color = UWmBody.UWTypes.Defaults.BodyColor "Color of cylinder" annotation(Dialog(colorSelector = true, enable = animation));
  ...
  final parameter UWFrames.Orientation R = Frames.from_nxy(r, {0, 1, 0}) "Orientation object from frame_a to frame spanned by cylinder axis and axis perpendicular to cylinder axis";
...
<a href=\"modelica://Modelica.Mechanics.MultiBody.Parts.Body\">MultiBody.Parts.Body</a>.</p></html>"), Icon(coordinateSystem(preserveAspectRatio = true, extent = {{-100, -100}, {100, 100}}, initialScale = 0.1, grid = {10, 10}), graphics = {Text(visible = true, textColor = {64, 64, 64}, extent = {{-150, 50}, {150, 90}}, textString = "%name"), Text(visible = true, textColor = {64, 64, 64}, extent = {{-150, -80}, {150, -50}}, textString = "r=%r"), Rectangle(visible = true, lineColor = {10, 90, 224}, fillColor = {156, 203, 255}, fillPattern = FillPattern.HorizontalCylinder, extent = {{-100, -40}, {100, 40}}, radius = 10), Text(visible = true, textColor = {64, 64, 64}, extent = {{-87, -12}, {-51, 13}}, textString = "a"), Text(visible = true, textColor = {64, 64, 64}, extent = {{51, -13}, {87, 12}}, textString = "b")}));
end BodyCylinder;

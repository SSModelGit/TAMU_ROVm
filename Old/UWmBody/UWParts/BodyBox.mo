...
model BodyBox "Rigid body with box shape. Mass and animation properties are computed from box data and density (12 potential states)"
  ...
  parameter UWmBody.UWTypes.Axis lengthDirection = to_unit1(r - r_shape) "Vector in length direction of box, resolved in frame_a" annotation(Evaluate = true);
  parameter UWmBody.UWTypes.Axis widthDirection = {0, 1, 0} "Vector in width direction of box, resolved in frame_a" annotation(Evaluate = true);
  ...
  input UWmBody.UWTypes.Color color = UWmBody.UWTypes.Defaults.BodyColor "Color of box" annotation(Dialog(colorSelector = true, enable = animation));
  input UWTypes.SpecularCoefficient specularCoefficient = world.defaultSpecularCoefficient "Reflection of ambient light (= 0: light is completely absorbed)" annotation(Dialog(enable = animation));
  ...
  parameter UWTypes.RotationSequence sequence_start = {1, 2, 3} "Sequence of rotations to rotate frame_a into frame_b at initial time" annotation(Evaluate = true, Dialog(tab = "Initialization"));
  ...
  parameter UWTypes.RotationSequence sequence_angleStates = {1, 2, 3} "Sequence of rotations to rotate world frame into frame_a around the 3 angles used as potential states" annotation(Evaluate = true, Dialog(tab = "Advanced", enable = not useQuaternions));
  parameter Modelica.SIunits.DimensionlessRatio c_d(min = 0, start = 1) annotation(Dialog);
  parameter Modelica.SIunits.Area cutA(min = 0, start = 1) annotation(Dialog);
  ...
  final parameter UWFrames.Orientation R = Frames.from_nxy(r, widthDirection) "Orientation object from frame_a to coordinates system spanned by r and widthDirection";
  final parameter SI.Position r_CM[3] = r_shape + normalizeWithAssert(lengthDirection) * length / 2 "Position vector from origin of frame_a to center of mass, resolved in frame_a";
  ...
  Body body(animation = false, r_CM = r_CM, m = m, d = density, I_11 = I[1, 1], I_22 = I[2, 2], I_33 = I[3, 3], I_21 = I[2, 1], I_31 = I[3, 1], I_32 = I[3, 2], sequence_start = sequence_start, angles_fixed = angles_fixed, angles_start = angles_start, w_0_fixed = w_0_fixed, w_0_start = w_0_start, z_0_fixed = z_0_fixed, z_0_start = z_0_start, useQuaternions = useQuaternions, sequence_angleStates = sequence_angleStates, enforceStates = false, c_d = c_d, cutA = cutA) annotation(Placement(transformation(extent = {{-30, -80}, {10, -40}}, origin = {0, 0}, rotation = 0), visible = true));
  FixedTranslation frameTranslation(r = r, animation = animation, shapeType = "box", r_shape = r_shape, lengthDirection = lengthDirection, widthDirection = widthDirection, length = length, width = width, height = height, color = color, specularCoefficient = specularCoefficient) annotation(Placement(transformation(extent = {{-30, -20}, {10, 20}})));
protected
...
equation
  ...
  connect(frame_a, body.frame_a) annotation(Line(points = {{-100, 0}, {-70, 0}, {-70, -60}, {-30, -60}}, color = {95, 95, 95}, thickness = 0.5, visible = true));
...
</html>"), Icon(coordinateSystem(preserveAspectRatio = true, extent = {{-100, -100}, {100, 100}}, initialScale = 0.1, grid = {10, 10}), graphics = {Polygon(visible = true, lineColor = {255, 255, 255}, fillColor = {10, 90, 224}, fillPattern = FillPattern.Solid, points = {{100, 40}, {100, -30}, {90, -40}, {90, 30}, {100, 40}}), Rectangle(visible = true, lineColor = {255, 255, 255}, fillColor = {71, 152, 255}, fillPattern = FillPattern.Solid, extent = {{-100, -40}, {90, 30}}), Polygon(visible = true, lineColor = {255, 255, 255}, fillColor = {156, 203, 255}, fillPattern = FillPattern.Solid, points = {{-100, 30}, {-90, 40}, {100, 40}, {90, 30}, {-100, 30}}), Text(visible = true, textColor = {64, 64, 64}, extent = {{-150, 50}, {150, 90}}, textString = "%name"), Text(visible = true, textColor = {64, 64, 64}, extent = {{-150, -80}, {150, -50}}, textString = "r=%r"), Text(visible = true, textColor = {64, 64, 64}, extent = {{52, -17}, {88, 8}}, textString = "b"), Text(visible = true, textColor = {64, 64, 64}, extent = {{-87, -13}, {-51, 12}}, textString = "a")}));
end BodyBox;

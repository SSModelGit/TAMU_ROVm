...
model Body "Rigid body with mass, inertia tensor and one frame connector (12 potential states)"
  ...
  UWmBody.UWInterfaces.Frame_a frame_a "Coordinate system fixed at body" annotation(Placement(transformation(extent = {{-116, -16}, {-84, 16}})));
  ...
  // parameter SI.Density d(min = 0, start = 1) "Density of rigid body, assuming uniformity, or m/v";
  ...
  input UWTypes.Color cylinderColor = sphereColor "Color of cylinder" annotation(Dialog(colorSelector = true, tab = "Animation", group = "if animation = true", enable = animation));
  ...
  final parameter UWFrames.Orientation R_start = UWmBody.UWFrames.axesRotations(sequence_start, angles_start, zeros(3)) "Orientation object from world frame to frame_a at initial time";
  ...
  /* Modelica.SIunits.Acceleration b_a[3] "Buoyant acceleration resolved in world frame";
  Modelica.SIunits.Force fld[3] "Viscous drag force, resolved in frame_a";
  Modelica.SIunits.Velocity scaV;
  // Modelica.SIunits.Velocity uniV[3];
...
protected
  ...
  parameter UWFrames.Quaternions.Orientation Q_start = Frames.to_Q(R_start) "Quaternion orientation object from world frame to frame_a at initial time";
  UWFrames.Quaternions.Orientation Q(start = Q_start, each stateSelect = if enforceStates then if useQuaternions then StateSelect.prefer else StateSelect.never else StateSelect.avoid) "Quaternion orientation object from world frame to frame_a (dummy value, if quaternions are not used as states)";
...
initial equation
  ...
  /* algorithm
  // viscous drag forces
  scaV := Modelica.Math.Vectors.length(v_0);
  uniV := smooth(1, Modelica.Math.Vectors.normalize(v_0));
  if abs((zeros(3) - diagonal(v_0))) < 1e-6*identity(3) then
    fld := 0;
  else
  fld := world.hydroDynDragForce(v = v_0, A = cutA, c = c_d);
  end if; */
equation
  ...
  // buoyant acceleration at center of mass resolved in world frame
  // b_a = world.buoyantAcceleration(d);
  ...
  annotation(Icon(coordinateSystem(preserveAspectRatio = true, extent = {{-100, -100}, {100, 100}}, initialScale = 0.1, grid = {10, 10}), graphics = {Rectangle(visible = true, lineColor = {10, 90, 224}, fillColor = {156, 203, 255}, fillPattern = FillPattern.HorizontalCylinder, extent = {{-100, -31}, {-3, 30}}, radius = 10), Text(visible = true, textColor = {64, 64, 64}, extent = {{-150, -100}, {150, -70}}, textString = "m=%m"), Text(visible = true, textColor = {64, 64, 64}, extent = {{-150, 70}, {150, 110}}, textString = "%name"), Ellipse(visible = true, lineColor = {10, 90, 224}, fillColor = {156, 203, 255}, fillPattern = FillPattern.Sphere, extent = {{-20, -60}, {100, 60}})}), Documentation(info = "<html>
<p>

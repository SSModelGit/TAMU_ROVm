...
model InitAngle "Internal model to initialize the angels for Joints.FreeMotionScalarInit"
  ...
  parameter UWmBody.UWTypes.RotationSequence sequence_start = {1, 2, 3} "Sequence of angle rotations";
  UWInterfaces.Frame_a frame_a annotation(Placement(transformation(extent = {{-116, -16}, {-84, 16}})));
  UWInterfaces.Frame_b frame_b annotation(Placement(transformation(extent = {{84, -16}, {116, 16}})));
  UWFrames.Orientation R_rel "Relative orientation object to rotate from frame_a to frame_b";
  UWFrames.Orientation R_rel_inv "Relative orientation object to rotate from frame_b to frame_a";
  Blocks.Interfaces.RealOutput angle[3](each final quantity = "Angle", each final unit = "rad") annotation(Placement(transformation(extent = {{-10, -10}, {10, 10}}, rotation = -90, origin = {0, -110})));
equation

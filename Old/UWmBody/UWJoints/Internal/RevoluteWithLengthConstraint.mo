...
model RevoluteWithLengthConstraint "Revolute joint where the rotation angle is computed from a length constraint (1 degree-of-freedom, no potential state)"
  ...
  Modelica.Mechanics.Rotational.Interfaces.Flange_a axis "1-dim. rotational flange that drives the joint" annotation(Placement(transformation(extent = {{10, 90}, {-10, 110}})));
  Modelica.Mechanics.Rotational.Interfaces.Flange_b bearing "1-dim. rotational flange of the drive bearing" annotation(Placement(transformation(extent = {{-50, 90}, {-70, 110}})));
  Modelica.Blocks.Interfaces.RealInput position_a[3](each final quantity = "Length", each final unit = "m") "Position vector from frame_a to frame_a side of length constraint, resolved in frame_a of revolute joint" annotation(Placement(transformation(extent = {{-140, -80}, {-100, -40}})));
  Modelica.Blocks.Interfaces.RealInput position_b[3](each final quantity = "Length", each final unit = "m") "Position vector from frame_b to frame_b side of length constraint, resolved in frame_b of revolute joint" annotation(Placement(transformation(extent = {{140, -80}, {100, -40}})));
  parameter Boolean animation = true "= true, if animation shall be enabled";
  parameter SI.Position lengthConstraint(start = 1) "Fixed length of length constraint";
  parameter UWmBody.UWTypes.Axis n = {0, 0, 1} "Axis of rotation resolved in frame_a (= same as in frame_b)" annotation(Evaluate = true);
  ...
  input UWTypes.Color cylinderColor = UWmBody.UWTypes.Defaults.JointColor "Color of cylinder representing the joint axis" annotation(Dialog(colorSelector = true, tab = "Animation", group = "if animation = true", enable = animation));
  ...
  UWFrames.Orientation R_rel "Relative orientation object from frame_a to frame_b";
...
protected
  ...
  function selectBranch "Determine branch which is closest to initial angle=0"
    ...
    input Modelica.SIunits.Angle angle_guess "Select the configuration such that at initial time |angle-angle_guess| is minimal (angle=0: frame_a and frame_b coincide)";
  ...
  protected
    ...
    Modelica.SIunits.Angle angle1 "solution 1 of nonlinear equation";
    Modelica.SIunits.Angle angle2 "solution 2 of nonlinear equation";
  algorithm
    ...
    B := 2 * r_b * cross(e, r_a);
    Modelica.Constants := r_a * r_a + r_b * r_b - L * L - 2 * e_r_b * e_r_a;
    k1 := A * A + B * B;
    k1a := k1 - Modelica.Constants * Modelica.Constants;
    assert(k1a > 1.e-10, "
        ...
        Try first to use another UWmBody.UWJoints.Assemblies.JointXXX component.
    ...
    k1b := max(k1a, 1.0e-12);
    k2 := sqrt(k1b);
    kcos1 := (-A * Modelica.Constants) + B * k2;
    ksin1 := (-B * Modelica.Constants) - A * k2;
    angle1 := atan2(ksin1, kcos1);
    kcos2 := (-A * Modelica.Constants) - B * k2;
    ksin2 := (-B * Modelica.Constants) + A * k2;
...
equation
  ...
  A = -2 * (r_b * r_a - e_r_b * e_r_a);
  B = 2 * r_b * cross(e, r_a);
  Modelica.Constants = r_a * r_a + r_b * r_b - lengthConstraint * lengthConstraint - 2 * e_r_b * e_r_a;
  k1 = A * A + B * B;
  k1a = k1 - Modelica.Constants * Modelica.Constants;
  assert(k1a > 1.e-10, "
      Singular position of loop (either no or two analytic solutions;
  ...
  k2 = sqrt(k1b);
  kcos_angle = (-A * Modelica.Constants) + (if positiveBranch then B else -B) * k2;
  ksin_angle = (-B * Modelica.Constants) + (if positiveBranch then -A else A) * k2;
  angle = Modelica.Math.atan2(ksin_angle, kcos_angle);
  annotation(Icon(coordinateSystem(preserveAspectRatio = true, extent = {{-100, -100}, {100, 100}}, initialScale = 0.1, grid = {10, 10}), graphics = {Rectangle(visible = true, lineColor = {64, 64, 64}, fillColor = {255, 255, 255}, fillPattern = FillPattern.HorizontalCylinder, extent = {{-30, -10}, {10, 10}}), Rectangle(visible = true, lineColor = {64, 64, 64}, fillColor = {255, 255, 255}, fillPattern = FillPattern.HorizontalCylinder, extent = {{-100, -60}, {-30, 60}}, radius = 10), Rectangle(visible = true, lineColor = {64, 64, 64}, fillColor = {255, 255, 255}, fillPattern = FillPattern.HorizontalCylinder, extent = {{30, -60}, {100, 60}}, radius = 10), Text(visible = true, textColor = {64, 64, 64}, extent = {{-139, -150}, {137, -110}}, textString = "%name"), Rectangle(visible = true, lineColor = {64, 64, 64}, extent = {{-100, -60}, {-30, 60}}, radius = 10), Rectangle(visible = true, lineColor = {64, 64, 64}, extent = {{30, -60}, {100, 60}}, radius = 10), Text(visible = true, textColor = {64, 64, 64}, extent = {{-142, -100}, {147, -70}}, textString = "n=%n"), Line(visible = true, points = {{-60, 60}, {-60, 90}}, color = {64, 64, 64}), Line(visible = true, points = {{-20, 70}, {-60, 70}}, color = {64, 64, 64}), Line(visible = true, points = {{-20, 80}, {-20, 60}}, color = {64, 64, 64}), Line(visible = true, points = {{20, 80}, {20, 60}}, color = {64, 64, 64}), Line(visible = true, points = {{20, 70}, {41, 70}}, color = {64, 64, 64}), Polygon(visible = true, lineColor = {64, 64, 64}, fillColor = {192, 192, 192}, fillPattern = FillPattern.Solid, points = {{-9, 30}, {10, 30}, {30, 50}, {-29, 50}, {-9, 30}}), Polygon(visible = true, lineColor = {64, 64, 64}, fillColor = {192, 192, 192}, fillPattern = FillPattern.Solid, points = {{10, 30}, {30, 50}, {30, -51}, {10, -31}, {10, 30}}), Rectangle(visible = true, lineColor = {64, 64, 64}, fillColor = {255, 255, 255}, fillPattern = FillPattern.VerticalCylinder, extent = {{-10, 50}, {10, 90}})}), Diagram(coordinateSystem(preserveAspectRatio = true, extent = {{-100, -100}, {100, 100}}), graphics = {Rectangle(extent = {{-100, -60}, {-30, 60}}, lineColor = {64, 64, 64}, fillPattern = FillPattern.HorizontalCylinder, fillColor = {255, 255, 255}, radius = 10), Rectangle(extent = {{-100, -60}, {-30, 60}}, lineColor = {64, 64, 64}, radius = 10), Rectangle(extent = {{-30, 10}, {10, -10}}, lineColor = {64, 64, 64}, fillColor = {192, 192, 192}, fillPattern = FillPattern.Solid), Rectangle(extent = {{30, -60}, {100, 60}}, lineColor = {64, 64, 64}, fillPattern = FillPattern.HorizontalCylinder, fillColor = {255, 255, 255}, radius = 10), Rectangle(extent = {{30, -60}, {100, 60}}, lineColor = {64, 64, 64}, radius = 10), Line(points = {{-60, 60}, {-60, 96}}), Line(points = {{-20, 70}, {-60, 70}}), Line(points = {{-20, 80}, {-20, 60}}), Line(points = {{20, 80}, {20, 60}}), Line(points = {{20, 70}, {41, 70}}), Polygon(points = {{-9, 30}, {10, 30}, {30, 50}, {-29, 50}, {-9, 30}}, lineColor = {64, 64, 64}, fillColor = {192, 192, 192}, fillPattern = FillPattern.Solid), Polygon(points = {{10, 30}, {30, 50}, {30, -51}, {10, -31}, {10, 30}}, lineColor = {64, 64, 64}, fillColor = {192, 192, 192}, fillPattern = FillPattern.Solid), Rectangle(extent = {{-10, 50}, {10, 100}}, lineColor = {64, 64, 64}, fillPattern = FillPattern.VerticalCylinder, fillColor = {192, 192, 192})}), Documentation(info = "<html>
<p>

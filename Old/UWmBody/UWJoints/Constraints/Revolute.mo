within UWmBody.UWJoints.Constraints;
...
model Revolute "Revolute cut-joint and translational directions may be constrained or released"
  ...
  parameter UWTypes.Axis n = {0, 1, 0} "Axis of rotation resolved in frame_a (= same as in frame_b)" annotation(Evaluate = true);
  ...
  input UWTypes.Color sphereColor = Types.Defaults.JointColor "Color of sphere representing the spherical joint" annotation(Dialog(colorSelector = true, group = "if animation = true", enable = animation));

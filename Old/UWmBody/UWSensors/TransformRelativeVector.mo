...
model TransformRelativeVector "Transform relative vector in to another frame"
  ...
  UWmBody.UWInterfaces.Frame_resolve frame_resolve if frame_r_in == UWmBody.UWTypes.ResolveInFrameAB.frame_resolve or frame_r_out == UWmBody.UWTypes.ResolveInFrameAB.frame_resolve "Coordinate system in which r_in or r_out is optionally resolved" annotation(Placement(transformation(extent = {{84, 64}, {116, 96}}), iconTransformation(extent = {{84, 65}, {116, 97}})));
  ...
  parameter UWmBody.UWTypes.ResolveInFrameAB frame_r_in = UWmBody.UWTypes.ResolveInFrameAB.frame_a "Frame in which vector r_in is resolved (world, frame_a, frame_b, or frame_resolve)";
  parameter UWmBody.UWTypes.ResolveInFrameAB frame_r_out = frame_r_in "Frame in which vector r_in shall be resolved and provided as r_out (world, frame_a, frame_b, or frame_resolve)";
protected
  UWmBody.UWSensors.Internal.BasicTransformRelativeVector basicTransformVector(frame_r_in = frame_r_in, frame_r_out = frame_r_out) annotation(Placement(transformation(extent = {{-10, -10}, {10, 10}})));
  UWmBody.UWInterfaces.ZeroPosition zeroPosition if not (frame_r_in == UWmBody.UWTypes.ResolveInFrameAB.frame_resolve or frame_r_out == UWmBody.UWTypes.ResolveInFrameAB.frame_resolve) annotation(Placement(transformation(extent = {{48, 10}, {68, 30}})));
equation

...
model Distance "Measure the distance between the origins of two frame connectors"
  ...
  input UWTypes.Color arrowColor = UWmBody.UWTypes.Defaults.SensorColor "Color of relative arrow from frame_a to frame_b" annotation(Dialog(colorSelector = true, group = "if animation = true", enable = animation));
...
protected
  UWmBody.UWVisualizers.Advanced.Arrow arrow(r = frame_a.r_0, r_head = frame_b.r_0 - frame_a.r_0, diameter = arrowDiameter, color = arrowColor, specularCoefficient = specularCoefficient) if world.enableAnimation and animation;
protected

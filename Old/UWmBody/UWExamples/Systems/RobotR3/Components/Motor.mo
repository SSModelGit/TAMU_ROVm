...
model Motor "Motor model including current controller of r3 motors"
  extends UWmBody.UWIcons.MotorIcon;
  ...
  Electrical.Analog.Basic.EMF emf(k = k, useSupport = false) annotation(Placement(transformation(extent = {{46, -10}, {66, 10}})));
  ...
  UWmBody.UWExamples.Systems.RobotR3.Components.AxisControlBus axisControlBus annotation(Placement(transformation(extent = {{60, -120}, {100, -80}})));
  Blocks.Math.Gain convert1(k(unit = "V/A") = 1) annotation(Placement(transformation(extent = {{-30, -56}, {-42, -44}})));
  Blocks.Math.Gain convert2(k(unit = "V/A") = 1) annotation(Placement(transformation(extent = {{-30, -101}, {-42, -89}})));
initial equation

...
model RevoluteConstraint "Body attached by one spring and revolute joint or constrained to environment"
  ...
  UWmBody.UWJoints.Revolute joint(stateSelect = StateSelect.never, n = {0, 1, 0}, phi(fixed = true), w(fixed = true)) annotation(Placement(transformation(extent = {{60, -30}, {40, -10}}, origin = {0, 10}, rotation = 0), visible = true));
  UWJoints.Constraints.Revolute constraint(n = joint.n) annotation(Placement(transformation(extent = {{60, 10}, {40, 30}}, origin = {0, 10}, rotation = 0), visible = true));
  ...
  UWJoints.FreeMotionScalarInit freeMotionScalarInit(use_angle = true, use_angle_d = true, angle_2(start = 0, fixed = true), angle_d_2(start = 0, fixed = true)) annotation(Placement(transformation(extent = {{40, 60}, {20, 80}}, origin = {0, 10}, rotation = 0), visible = true));
equation

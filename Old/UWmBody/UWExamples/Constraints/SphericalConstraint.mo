...
model SphericalConstraint "Body attached by one spring and spherical joint or constrained to environment"
  extends Modelica.Icons.Example;
  parameter Boolean animation = true "= true, if animation shall be enabled";
  UWJoints.Spherical joint(angles_fixed = true, w_rel_a_fixed = true, enforceStates = true) annotation(Placement(transformation(extent = {{60, -30}, {40, -10}}, origin = {0, 10}, rotation = 0), visible = true));
  UWJoints.Constraints.Spherical constraint annotation(Placement(transformation(extent = {{60, 10}, {40, 30}}, origin = {0, 10}, rotation = 0), visible = true));
  ...
  UWmBody.UWParts.FixedRotation fixedRotation(r = {0.2, -0.3, 0.2}, rotationType = UWmBody.UWTypes.RotationTypes.PlanarRotationSequence, angles = {10, 55, 68}) annotation(Placement(transformation(extent = {{-10, -10}, {10, 10}}, rotation = 90, origin = {80, -40}), visible = true));
  ...
  UWJoints.FreeMotionScalarInit freeMotionScalarInit(use_angle = true, use_angle_d = true, angle_1(fixed = true), angle_2(fixed = true), angle_3(fixed = true), angle_d_1(fixed = true), angle_d_2(fixed = true), angle_d_3(fixed = true)) annotation(Placement(transformation(extent = {{38, 60}, {18, 80}}, origin = {2, 10}, rotation = 0), visible = true));
equation
  ...
  connect(freeMotionScalarInit.frame_a, fixedRotation.frame_b) annotation(Line(points = {{-26.667, 36.667}, {13.333, 36.667}, {13.333, -73.333}}, color = {95, 95, 95}, thickness = 0.5, visible = true, origin = {66.667, 43.333}));

...
model Surfaces "Demonstrate the visualization of a sine surface, as well as a torus and a wheel constructed from a surface"
  ...
  Real wz = time;
  UWmBody.UWVisualizers.Advanced.Surface surface(redeclare function surfaceCharacteristic = UWmBody.UWExamples.Elementary.Utilities.sineSurface(x_min = x_min, x_max = x_max, y_min = y_min, y_max = y_max, z_min = z_min, z_max = z_max, wz = wz), multiColoredSurface = false, nu = 50, nv = 50) annotation(Placement(transformation(extent = {{-72, -54}, {-52, -34}}, origin = {12, -16}, rotation = 0), visible = true));
  inner UWWorld world(axisLength = 1, n = {0, 0, -1}) annotation(Placement(transformation(extent = {{-80, 20}, {-60, 40}}, origin = {20, -20}, rotation = 0), visible = true));
  UWVisualizers.Torus torus annotation(Placement(transformation(extent = {{32, 20}, {52, 40}}, origin = {28, -20}, rotation = 0), visible = true));
  UWJoints.Prismatic prismatic(useAxisFlange = true, animation = false, v(fixed = true)) annotation(Placement(transformation(extent = {{-38, 20}, {-18, 40}}, origin = {18, -20}, rotation = 0), visible = true));
  ...
  Blocks.Sources.Sine sine(amplitude = 2, freqHz = 0.5) annotation(Placement(transformation(extent = {{-98, 70}, {-78, 90}}, origin = {18, -30}, rotation = 0), visible = true));
  UWVisualizers.Ground ground(groundColor = {215, 215, 215}, length = 4) annotation(Placement(transformation(extent = {{-72, -16}, {-52, 4}}, origin = {12, -14}, rotation = 0), visible = true));
  UWParts.FixedTranslation fixedTranslation1(r = {0, -1.3, torus.ro + torus.ri}, animation = false) annotation(Placement(transformation(extent = {{-4, 20}, {16, 40}}, origin = {24, -20}, rotation = 0), visible = true));
  UWParts.FixedTranslation fixedTranslation2(animation = false, r = {0, -1.6, wheel.rTire}) annotation(Placement(transformation(extent = {{-4, 50}, {16, 70}}, origin = {24, -20}, rotation = 0), visible = true));
  UWVisualizers.VoluminousWheel wheel annotation(Placement(transformation(extent = {{32, 50}, {52, 70}}, origin = {28, -20}, rotation = 0), visible = true));
  UWVisualizers.PipeWithScalarField pipeWithScalarField(rOuter = 0.3, length = 1, T_min = 0, T_max = 2, T = sin(Modelica.Constants.pi * pipeWithScalarField.xsi) * cos(Modelica.Constants.pi * time) .+ 1, n_colors = 32) annotation(Placement(transformation(extent = {{14, -30}, {34, -10}}, origin = {16, -10}, rotation = 0), visible = true));
  UWParts.FixedTranslation fixedTranslation3(animation = false, r = {0, -2.2, 0}) annotation(Placement(transformation(extent = {{-20, -30}, {0, -10}}, origin = {10, -10}, rotation = 0), visible = true));
equation
...
<a href=\"modelica://UWmBody.Visualizers.Advanced.Surface\">Surface</a>
...
<a href=\"modelica://UWmBody.Visualizers.Torus\">Torus</a>,
the \"wheel\" surface is an instance of
<a href=\"modelica://UWmBody.Visualizers.VoluminousWheel\">VoluminousWheel</a>,
and the \"pipeWithScalarField surface is an instance of
<a href=\"modelica://UWmBody.Visualizers.PipeWithScalarField\">PipeWithScalarField</a>.

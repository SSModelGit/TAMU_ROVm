...
model UserDefinedGravityField "Demonstrate the modeling of a user-defined gravity field"
  ...
  inner UWmBody.UWWorld world(gravityType = UWmBody.UWTypes.GravityTypes.NoGravity, redeclare function gravityAcceleration = UWmBody.UWExamples.Elementary.Utilities.theoreticalNormalGravityWGS84(phi = geodeticLatitude), axisLength = 10, nominalLength = 10) annotation(Placement(transformation(extent = {{-80, -20}, {-60, 0}}, origin = {10, -30}, rotation = 0), visible = true));
  UWJoints.Revolute rev(n = {0, 0, 1}, useAxisFlange = true, phi(fixed = true), w(fixed = true)) annotation(Placement(transformation(extent = {{-14, 20}, {6, 40}}, origin = {4, -30}, rotation = 0), visible = true));
  ...
  UWParts.Body body(r_CM = {10, 0, 0}, m = 1000.0, sphereDiameter = 1) annotation(Placement(transformation(extent = {{26, 20}, {46, 40}}, origin = {4, -30}, rotation = 0), visible = true));
  UWParts.FixedTranslation fixedTranslation(r = {0, height, 0}, width = 0.3) annotation(Placement(transformation(extent = {{-10, -10}, {10, 10}}, rotation = 90, origin = {-30, -20}), visible = true));
equation
...
<a href=\"modelica://UWmBody.Examples.Elementary.Utilities.theoreticalNormalGravityWGS84\">theoreticalNormalGravityWGS84</a>

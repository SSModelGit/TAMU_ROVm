within UWmBody.UWExamples.Elementary.Utilities;

function sineSurface "Function defining the characteristic of a moving sine in three dimensions"
  extends UWmBody.UWInterfaces.partialSurfaceCharacteristic;
...
algorithm
  ...
  if multiColoredSurface then
    Modelica.Constants := array({(Z[i, j] + 1) * 200, 255, 0} for j in 1:nv, i in 1:nu);
  end if;
  annotation(Documentation(info = "<html>
<p>
Function defining the characteristics of a moving sine in three dimensions.
This function is used in example
<a href=\"UWmBody.Examples.Elementary.Surfaces\">Elementary.Surfaces</a>.
</p>
</html>"));
end sineSurface;

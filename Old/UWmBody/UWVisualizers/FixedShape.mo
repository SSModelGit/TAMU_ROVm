...
model FixedShape "Visualizing an elementary shape with dynamically varying shape attributes (has one frame connector)"
  ...
  parameter UWTypes.ShapeType shapeType = "box" "Type of shape" annotation(Dialog(group = "if animation = true", enable = animation));
  ...
  input UWTypes.Axis lengthDirection = {1, 0, 0} "Vector in length direction of shape, resolved in frame_a" annotation(Evaluate = true, Dialog(group = "if animation = true", enable = animation));
  input UWTypes.Axis widthDirection = {0, 1, 0} "Vector in width direction of shape, resolved in frame_a" annotation(Evaluate = true, Dialog(group = "if animation = true", enable = animation));
  ...
  input UWmBody.UWTypes.Color color = {0, 128, 255} "Color of shape" annotation(Dialog(colorSelector = true, group = "if animation = true", enable = animation));
  input UWTypes.ShapeExtra extra = 0.0 "Additional data for cylinder, cone, pipe, gearwheel and spring" annotation(Dialog(group = "if animation = true", enable = animation));
...
</html>"), Icon(coordinateSystem(preserveAspectRatio = true, extent = {{-100, -100}, {100, 100}}, initialScale = 0.1, grid = {5, 5}), graphics = {Polygon(visible = true, origin = {0, 5}, lineColor = {0, 127, 255}, fillColor = {71, 152, 255}, fillPattern = FillPattern.Solid, points = {{-100, -55}, {0, -40}, {50, -60}, {50, 30}, {0, 10}, {-100, 40}}), Polygon(visible = true, origin = {0, 5}, lineColor = {255, 255, 255}, fillColor = {156, 203, 255}, fillPattern = FillPattern.Solid, points = {{-75, 60}, {25, 30}, {75, 50}, {50, 30}, {0, 10}, {-100, 40}}), Polygon(visible = true, origin = {0, 5}, lineColor = {255, 255, 255}, fillColor = {10, 90, 224}, fillPattern = FillPattern.Solid, points = {{75, 50}, {50, 30}, {50, -60}, {75, -40}}), Text(visible = true, textColor = {64, 64, 64}, extent = {{-150, -95}, {150, -65}}, textString = "%shapeType"), Text(visible = true, origin = {0, 5}, textColor = {64, 64, 64}, extent = {{-150, 60}, {150, 100}}, textString = "%name")}));
end FixedShape;

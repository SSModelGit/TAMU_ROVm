...
function pipeWithScalarField "Function defining the surface characteristic of a pipe where a scalar field value is displayed with color along the pipe axis"
  extends UWmBody.UWInterfaces.partialSurfaceCharacteristic(final multiColoredSurface = true);
...
algorithm
  ...
  for i in 1:nu loop
    ...
    Ci := UWmBody.UWVisualizers.Colors.scalarToColor(Ti, T_min, T_max, colorMap);
    for j in 1:nv loop

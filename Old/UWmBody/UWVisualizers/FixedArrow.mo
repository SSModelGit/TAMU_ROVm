...
model FixedArrow "Visualizing an arrow with dynamically varying size in frame_a"
  ...
  input UWTypes.Axis n = {1, 0, 0} "Vector in arrow direction, resolved in frame_a" annotation(Dialog(group = "if animation = true", enable = animation));
  ...
  input UWTypes.Color color = {0, 0, 255} "Color of arrow" annotation(Dialog(colorSelector = true, group = "if animation = true", enable = animation));

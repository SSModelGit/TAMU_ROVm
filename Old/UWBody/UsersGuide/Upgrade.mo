...
\"new\" UWBody library. This script is also
automatically invoked. Since the differences between the \"old\" and the
\"new\" UWBody library are so large, not everything is
...
Frame connectors of the UWBody library, but the equations
...
functions from library UWBody.Frames):
</p>
...
<p>
<b>Upgrade from UWBody 0.99 (and earlier) to 1.0 (and later)</b>
</p>
<p>
The conversion from UWBody 0.99 to 1.0 does not work in some rare
...
<p>
The definition of the UWBody.Frames.Orientation object has changed.
In 0.99 this was just an alias type for a transformation matrix
(now UWBody.Frames.TransformationMatrices.Orientation).

...
model RelativeAngles "Measure relative angles between two frame connectors"
  ...
  parameter UWBody.Types.RotationSequence sequence(min = {1, 1, 1}, max = {3, 3, 3}) = {1, 2, 3} "Angles are returned to rotate frame_a around axes sequence[1], sequence[2] and finally sequence[3] into frame_b" annotation(Evaluate = true);
...
equation
  ...
  frame_b.f = zeros(3);
  frame_b.t = zeros(3);
  R_rel = UWBody.Frames.relativeRotation(frame_a.R, frame_b.R);
  angles = UWBody.Frames.axesRotationsAngles(R_rel, sequence, guessAngle1);

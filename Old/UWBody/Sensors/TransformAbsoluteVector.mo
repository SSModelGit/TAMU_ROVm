...
model TransformAbsoluteVector "Transform absolute vector in to another frame"
  ...
  UWBody.Interfaces.Frame_a frame_a "Coordinate system from which absolute kinematic quantities are measured" annotation(Placement(transformation(extent = {{-116, -16}, {-84, 16}})));
  UWBody.Interfaces.Frame_resolve frame_resolve if frame_r_in == UWBody.Types.ResolveInFrameA.frame_resolve or frame_r_out == UWBody.Types.ResolveInFrameA.frame_resolve "Coordinate system in which r_in or r_out is optionally resolved" annotation(Placement(transformation(extent = {{84, -16}, {116, 16}}), iconTransformation(extent = {{84, -15}, {116, 17}})));
  ...
  parameter UWBody.Types.ResolveInFrameA frame_r_in = UWBody.Types.ResolveInFrameA.frame_a "Frame in which vector r_in is resolved (world, frame_a, or frame_resolve)";
  parameter UWBody.Types.ResolveInFrameA frame_r_out = frame_r_in "Frame in which vector r_in shall be resolved and provided as r_out (world, frame_a, or frame_resolve)";
protected
  ...
  UWBody.Interfaces.ZeroPosition zeroPosition if not (frame_r_in == UWBody.Types.ResolveInFrameA.frame_resolve or frame_r_out == UWBody.Types.ResolveInFrameA.frame_resolve) annotation(Placement(transformation(extent = {{40, 18}, {60, 38}})));
equation

...
model WorldTorque "External torque acting at frame_b, defined by 3 input signals and resolved in frame world, frame_b or frame_resolve"
  ...
  parameter UWBody.Types.ResolveInFrameB resolveInFrame = UWBody.Types.ResolveInFrameB.world "Frame in which input torque is resolved (1: world, 2: frame_b, 3: frame_resolve)";
  ...
  input Types.Color color = UWBody.Types.Defaults.TorqueColor "Color of arrow" annotation(Dialog(colorSelector = true, group = "if animation = true", enable = animation));
...
protected
  Interfaces.ZeroPosition zeroPosition if not resolveInFrame == UWBody.Types.ResolveInFrameB.frame_resolve annotation(Placement(transformation(extent = {{20, 10}, {40, 30}})));
equation

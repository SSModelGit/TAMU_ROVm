...
model Pendulum "Simple pendulum with one revolute joint and one body"
  ...
  inner UWBody.World world(gravityType = UWBody.Types.GravityTypes.UniformGravity) annotation(Placement(transformation(extent = {{-60, 0}, {-40, 20}}, origin = {10, -10}, rotation = 0), visible = true));
  UWBody.Joints.Revolute rev(n = {0, 0, 1}, useAxisFlange = true, phi(fixed = true), w(fixed = true)) annotation(Placement(transformation(extent = {{-20, 0}, {0, 20}}, origin = {10, -10}, rotation = 0), visible = true));
  ...
  UWBody.Parts.Body body(m = 1.0, r_CM = {0.5, 0, 0}) annotation(Placement(transformation(extent = {{20, 0}, {40, 20}}, origin = {10, -10}, rotation = 0), visible = true));
equation

...
model PendulumWithSpringDamper "Simple spring/damper/mass system"
  ...
  inner UWBody.World world(axisLength = 0.6) annotation(Placement(transformation(extent = {{-80, 20}, {-60, 40}}, origin = {0, -5}, rotation = 0), visible = true));
  UWBody.Parts.Body body1(m = 1, animation = animation, I_11 = 1, I_22 = 1, I_33 = 1, r_CM = {0, 0, 0}, cylinderDiameter = 0.05, sphereDiameter = 0.2) annotation(Placement(transformation(origin = {70, 25}, extent = {{10, -10}, {-10, 10}}, rotation = 180), visible = true));
  UWBody.Parts.FixedTranslation bar1(animation = animation, r = {0.3, 0, 0}) annotation(Placement(transformation(extent = {{-46, 20}, {-26, 40}}, origin = {-4, -5}, rotation = 0), visible = true));
  UWBody.Forces.Spring spring1(coilWidth = 0.01, numberOfWindings = 5, c = 20, s_unstretched = 0.2) annotation(Placement(transformation(extent = {{0, -46}, {20, -26}}, origin = {5, 11}, rotation = 0), visible = true));
  UWBody.Forces.Damper damper1(d = 1, length_a = 0.1, diameter_a = 0.08, animation = false) annotation(Placement(transformation(extent = {{0, -20}, {20, 0}}, origin = {5, 10}, rotation = 0), visible = true));
  UWBody.Joints.Revolute revolute(phi(fixed = true), w(fixed = true)) annotation(Placement(transformation(extent = {{-12, 20}, {8, 40}}, origin = {2, -5}, rotation = 0), visible = true));
  UWBody.Joints.Prismatic prismatic(boxWidth = 0.04, boxColor = {255, 65, 65}, s(fixed = true, start = 0.5), v(fixed = true)) annotation(Placement(transformation(extent = {{20, 20}, {40, 40}}, origin = {0, -5}, rotation = 0), visible = true));
equation

...
model Engine1b "Model of one cylinder engine with gas force and preparation for assembly joint JointRRP"
  ...
  extends Modelica.Utilities.Engine1bBase(Inertia(w(start = 0)));
  Joints.RevolutePlanarLoopConstraint B2(n = {1, 0, 0}, cylinderLength = 0.02, cylinderDiameter = 0.05) annotation(Placement(transformation(extent = {{40, 20}, {60, 40}})));
  UWBody.Joints.Revolute B1(n = {1, 0, 0}, cylinderLength = 0.02, cylinderDiameter = 0.05) annotation(Placement(transformation(extent = {{40, -20}, {60, 0}})));
  UWBody.Joints.Prismatic Cylinder(useAxisFlange = true, boxWidth = 0.02, n = {0, -1, 0}) annotation(Placement(transformation(origin = {50, 95}, extent = {{-10, -10}, {10, 10}}, rotation = 270), visible = true));
  ...
  Parts.FixedTranslation Rod3(r = {0, -0.1, 0}, animation = false) annotation(Placement(transformation(origin = {50, 60}, extent = {{10, -10}, {-10, 10}}, rotation = 90), visible = true));
equation
...
It is similar to
<a href=\"modelica://UWBody.Examples.Loops.Engine1a#diagram\">Loops.Engine1a</a>.
...
assembly joint \"UWBody.Joints.Assemblies.<b>JointRRP</b>\".
This exchange of the kinematic loop is shown in
<a href=\"modelica://UWBody.Examples.Loops.Engine1b_analytic#diagram\">Loops.Engine1b_analytic</a>.

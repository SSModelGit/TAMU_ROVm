...
model BevelGear1D "Demonstrates the usage of a BevelGear1D model and how to calculate the power of such an element"
  ...
  parameter UWBody.Types.Axis na = {1, 0, 0} "Axis of rotation of left gear axis";
  parameter UWBody.Types.Axis nb = {0, 1, 0} "Axis of rotation of right gear axis";
  inner UWBody.World world(final driveTrainMechanics3D = true) annotation(Placement(transformation(extent = {{-80, -20}, {-60, 0}}, origin = {-20, -20}, rotation = 0), visible = true));
  UWBody.Parts.Rotor1D inertia1(J = 1.1, a(fixed = false), phi(fixed = true, start = 0), w(fixed = true, start = 0), n = na) annotation(Placement(transformation(extent = {{-30, 60}, {-10, 80}}, origin = {0, -20}, rotation = 0), visible = true));
  UWBody.Parts.Rotor1D inertia2(J = 18.2, n = nb) annotation(Placement(transformation(extent = {{30, 60}, {50, 80}}, origin = {-0, -20}, rotation = 0), visible = true));
  UWBody.Parts.BevelGear1D bevelGear(ratio = 10, n_a = na, n_b = nb) annotation(Placement(transformation(extent = {{0, 60}, {20, 80}}, origin = {0, -20}, rotation = 0), visible = true));
  UWBody.Joints.Revolute revolute1(useAxisFlange = true, n = {1, 0, 0}, stateSelect = StateSelect.always, phi(fixed = true), w(fixed = true)) annotation(Placement(transformation(extent = {{-46, 0}, {-26, -20}}, origin = {-14, -20}, rotation = 0), visible = true));
  UWBody.Joints.Revolute revolute2(useAxisFlange = true, n = {0, 1, 0}, stateSelect = StateSelect.always, phi(fixed = true), w(fixed = true)) annotation(Placement(transformation(extent = {{0, 0}, {20, -20}}, origin = {-10, -20}, rotation = 0), visible = true));
  UWBody.Joints.Revolute revolute3(useAxisFlange = true, n = {0, 0, 1}, stateSelect = StateSelect.always, phi(fixed = true), w(fixed = true)) annotation(Placement(transformation(extent = {{46, 0}, {66, -20}}, origin = {-6, -20}, rotation = 0), visible = true));
  ...
  UWBody.Parts.Mounting1D mounting1D(n = na) annotation(Placement(transformation(extent = {{-72, 46}, {-52, 66}}, origin = {-8, -36}, rotation = 0), visible = true));
  ...
  UWBody.Parts.BodyBox bodyBox(r = {0.1, 0.1, 0.1}, length = 0.1, width = 0.1) annotation(Placement(transformation(extent = {{76, -20}, {96, 0}}, origin = {4, -20}, rotation = 0), visible = true));
  Sensors.AbsoluteAngularVelocity sensor1(resolveInFrame = UWBody.Types.ResolveInFrameA.frame_a) annotation(Placement(transformation(extent = {{62, 42}, {82, 62}}, origin = {-2, -22}, rotation = 0), visible = true));
  Sensors.CutTorque sensor2(resolveInFrame = UWBody.Types.ResolveInFrameA.frame_a, animation = false) annotation(Placement(transformation(extent = {{10, -10}, {-10, 10}}, rotation = -90, origin = {10, 20}), visible = true));

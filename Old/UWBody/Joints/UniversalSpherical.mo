...
model UniversalSpherical "Universal - spherical joint aggregation (1 constraint, no potential states)"
  ...
  parameter UWBody.Types.Axis n1_a = {0, 0, 1} "Axis 1 of universal joint resolved in frame_a (axis 2 is orthogonal to axis 1 and to rod)" annotation(Evaluate = true);
  ...
  input Types.Color sphereColor = UWBody.Types.Defaults.JointColor "Color of spheres representing the universal and the spherical joint" annotation(Dialog(colorSelector = true, tab = "Animation", group = "if animation = true", enable = animation));
  ...
  input Types.Color rodColor = UWBody.Types.Defaults.RodColor "Color of rod shape connecting the universal and the spherical joints" annotation(Dialog(colorSelector = true, tab = "Animation", group = "if animation = true", enable = animation));
...
equation
  ...
  assert(length2_n2_a > 1.e-10, "
    A UWBody.Joints.UniversalSpherical joint (consisting of
    ...
    use instead UWBody.Joints.SphericalSpherical, if this is
...
<pre>
    UWBody.Joints.UniversalSpherical jointUS(rRod_ia={1.2, 1, 0.2});

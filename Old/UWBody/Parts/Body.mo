...
model Body "Rigid body with mass, inertia tensor and one frame connector (12 potential states)"
  ...
  UWBody.Interfaces.Frame_a frame_a "Coordinate system fixed at body" annotation(Placement(transformation(extent = {{-116, -16}, {-84, 16}})));
  ...
  input Types.Color sphereColor = UWBody.Types.Defaults.BodyColor "Color of sphere" annotation(Dialog(colorSelector = true, tab = "Animation", group = "if animation = true", enable = animation));
  ...
  final parameter Frames.Orientation R_start = UWBody.Frames.axesRotations(sequence_start, angles_start, zeros(3)) "Orientation object from world frame to frame_a at initial time";

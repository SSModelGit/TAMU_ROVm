...
model PointMass "Rigid body where body rotation and inertia tensor is neglected (6 potential states)"
  import UWBody.Types;
  Interfaces.Frame_a frame_a "Coordinate system fixed at center of mass point" annotation(Placement(transformation(extent = {{-16, -16}, {16, 16}})));
...
equation
  ...
  if Connections.isRoot(frame_a.R) then
    assert(cardinality(frame_a) == 0, "
      A UWBody.Parts.PointMass model is connected in
      ...
      UWBody.Joints.FreeMotion joint.

...
model DoubleArrow "Visualizing a double arrow with variable size; all data have to be set as modifiers (see info layer)"
  ...
  input UWBody.Types.Color color = UWBody.Types.Defaults.ArrowColor "Color of double arrow" annotation(Dialog(colorSelector = true));
...
protected
  ...
  SI.Length headLength = noEvent(max(0, min(length, diameter * UWBody.Types.Defaults.ArrowHeadLengthFraction)));
  SI.Length headWidth = noEvent(max(0, diameter * UWBody.Types.Defaults.ArrowHeadWidthFraction));
  SI.Length arrowLength = noEvent(max(0, length - 1.5 * diameter * UWBody.Types.Defaults.ArrowHeadLengthFraction));

within UWBody.Visualizers.Advanced.SurfaceCharacteristics;

function pipeWithScalarField "Function defining the surface characteristic of a pipe where a scalar field value is displayed with color along the pipe axis"
  extends UWBody.Interfaces.partialSurfaceCharacteristic(final multiColoredSurface = true);
...
algorithm
  ...
  for i in 1:nu loop
    ...
    Ci := UWBody.Visualizers.Colors.scalarToColor(Ti, T_min, T_max, colorMap);
    for j in 1:nv loop
      beta := 2 * Modelica.Constants.pi * (j - 1) / (nv - 1);
      X[i, j] := length * xsi_i;
      Y[i, j] := rOuter * sin(beta);
      Z[i, j] := rOuter * cos(beta);
      Modelica.Constants[i, j, :] := Ci;
    end for;
  end for;
...
<a href=\"modelica://UWBody.Visualizers.Advanced.PipeWithScalarField\">PipeWithScalarField</a>.
...
<a href=\"modelica://UWBody.Visualizers.Colors.ColorMaps\">MultiBody.Visualizers.Colors.ColorMaps</a>
...
<a href=\"modelica://UWBody.Visualizers.Colors.colorMapToSvg\">MultiBody.Visualizers.Colors.colorMapToSvg</a>.
...
<a href=\"modelica://UWBody.Visualizers.Colors.colorMapToSvg\">Mechanics.MultiBody.Visualizers.Colors.colorMapToSvg</a>
using the following call:
</p>

<blockquote>
<pre>
colorMapToSvg(UWBody.Visualizers.Colors.ColorMaps.jet(),

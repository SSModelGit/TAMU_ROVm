within UWBody.Visualizers.Colors.ColorMaps;

function jet "Returns the \"jet\" color map"
  extends UWBody.Interfaces.partialColorMap;
protected
  Real a = ceil(n_colors / 4);
  Real d = 1 / a;
  Integer b = integer(ceil(a / 2));
  Integer c = integer(floor(a / 2));
  Real v1[:] = array(1 - (b - i) * d for i in 1:b);
  Real v2[:] = 0 + d:d:1;
  Real v3[:] = 1 - d:(-d):0;
  Real v4[:] = array(0.5 + (c - i) * d for i in 1:c);
...
<h4>See also</h4>
<a href=\"modelica://UWBody.Visualizers.Colors.ColorMaps\">ColorMaps</a>,
<a href=\"modelica://UWBody.Visualizers.Colors.colorMapToSvg\">colorMapToSvg</a>,
<a href=\"modelica://UWBody.Visualizers.Colors.scalarToColor\">scalarToColor</a>.
</html>"));
end jet;

...
void ROSClientCall_Array_U(double time, int portno, double *query, double *res)
{
    printf("time: %f\n", time); // time keeper
    ...
    double buf[sizeof(query)];
    memcpy(buf, query, sizeof(query)); // keeps the input values in a double array buffer

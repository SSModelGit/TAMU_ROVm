...
model BasicBody
  ...
  Modelica.Mechanics.MultiBody.Forces.WorldForceAndTorque buoyantField(animation = false) "External force due to fields" annotation(Placement(visible = true, transformation(origin = {-51.931, 60}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
  ...
  SI.Velocity v_cross[3](start = {0, 0, 0}, each stateSelect = if enforceStates then StateSelect.always else StateSelect.avoid) "r_CM x w_a - in frame_a" annotation(Dialog(tab = "Initialization", showStartAttribute = true));
  SI.Velocity v_minus[3](start = {0, 0, 0}, each stateSelect = if enforceStates then StateSelect.always else StateSelect.avoid) "frame_a.v_0 - v_cross -- in world frame" annotation(Dialog(tab = "Initialization", showStartAttribute = true));
  ...
  Modelica.Mechanics.MultiBody.Forces.WorldForceAndTorque dragField(animation = false) "External drag force + torque due to fields" annotation(Placement(visible = true, transformation(origin = {-51.957, 28.244}, extent = {{-10, -10}, {10, 10}}, rotation = 0)));
protected
  // Fields
...
equation
  ...
  v_cross = cross(r_CM, w_a);
  // v_cross = zeros(3);
  v_minus = body.v_0 - Frames.resolve1(frame_a.R, v_cross);
  b_f = waterField.waterBuoyantForce(d = density, m = body.m);
  f_d = waterField.waterDragForce(v = v_minus, c = c_d, A = A);
  // f_d = waterField.waterDragForce(v = body.v_0, c = c_d, A = A);
  t_d = Frames.resolve1(frame_a.R, waterField.waterDragTorque(w = w_a, k = k_d));
  buoyantField.force = b_f;
  buoyantField.torque = cross(Frames.resolve1(frame_a.R, r_CM), buoyantField.force);
  dragField.force = f_d;
  dragField.torque = t_d + cross(Frames.resolve1(frame_a.R, r_CM), dragField.force);
  connect(frame_a, body.frame_a) annotation(Line(visible = true, origin = {-55, 0}, points = {{-45, 0}, {45, 0}}, color = {95, 95, 95}));
  connect(buoyantField.frame_b, body.frame_a) annotation(Line(visible = true, origin = {-19.489, 30}, points = {{-22.442, 30}, {6.476, 30}, {6.476, -30}, {9.489, -30}}, color = {95, 95, 95}));
  connect(dragField.frame_b, body.frame_a) annotation(Line(visible = true, origin = {-19.495, 14.122}, points = {{-22.461, 14.122}, {6.483, 14.122}, {6.483, -14.122}, {9.495, -14.122}}, color = {95, 95, 95}));
